...
import { CheckIcon, ChevronDownIcon, LoadingIcon, CancelIcon } from "@hugeicons/core-free-icons";
  ...
  return (
    ...
    >
      <div className="relative">
        <Combobox.Chips className="flex min-h-8 w-full flex-wrap items-center gap-1 rounded-md border border-input bg-transparent px-1 py-1 focus-within:border-ring focus-within:ring-3 focus-within:ring-ring/50">
          <Combobox.Value>
      ...
      <Combobox.Portal>
        <Combobox.Positioner sideOffset={4} className="z-50 outline-none">
          <Combobox.Popup className="max-h-60 w-[var(--anchor-width)] overflow-y-auto rounded-md border border-line bg-surface p-1 shadow-lg">
            <Combobox.Empty>
            ...
            <Combobox.List className="flex flex-col gap-0.5 outline-none">
              {(item: RoleItem) => (
                ...
                >
                  <span className="grid h-4 w-4 shrink-0 place-items-center rounded border border-line text-ink-1 group-data-[selected]:border-brand group-data-[selected]:bg-brand-fill group-data-[selected]:text-white">
                    <HugeiconsIcon icon={CheckIcon} size={12} className="opacity-0 group-data-[selected]:opacity-100" />
                  </span>
