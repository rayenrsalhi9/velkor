import { describe, it, expect, afterEach, vi } from "vitest";
import { render, screen } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import DeleteDocumentDialog from "./DeleteDocumentDialog";
import { DOCUMENTS, jsonResponse } from "@/test/fixtures";

vi.mock("sonner", () => ({
  toast: { success: vi.fn(), error: vi.fn() },
}));

import { toast } from "sonner";

function renderDialog(over: Record<string, unknown> = {}) {
  const onOpenChange = vi.fn();
  const onDeleted = vi.fn();
  render(
    <DeleteDocumentDialog
      document={DOCUMENTS[0]}
      onOpenChange={onOpenChange}
      onDeleted={onDeleted}
      {...over}
    />,
  );
  return { onOpenChange, onDeleted };
}

describe("DeleteDocumentDialog", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    vi.clearAllMocks();
  });

  it("deletes the document on confirm", async () => {
    const user = userEvent.setup();
    const { onOpenChange, onDeleted } = renderDialog();
    vi.stubGlobal(
      "fetch",
      vi.fn(() => jsonResponse(200, {})),
    );
    await user.click(
      await screen.findByRole("button", { name: "Delete document" }),
    );
    await vi.waitFor(() => expect(onDeleted).toHaveBeenCalled());
    expect(onOpenChange).toHaveBeenCalledWith(false);
    expect(toast.success).toHaveBeenCalledWith("Document deleted", expect.anything());
    const deleteCall = vi.mocked(fetch).mock.calls.find(
      ([url, init]) => url === "/api/documents/d1" && init?.method === "DELETE",
    );
    expect(deleteCall).toBeDefined();
  });

  it("is closed when no document is set", () => {
    renderDialog({ document: null });
    expect(
      screen.queryByRole("button", { name: "Delete document" }),
    ).not.toBeInTheDocument();
  });

  it("shows an error when deletion fails", async () => {
    const user = userEvent.setup();
    renderDialog();
    vi.stubGlobal(
      "fetch",
      vi.fn(() => jsonResponse(403, { error: "Forbidden" })),
    );
    await user.click(
      await screen.findByRole("button", { name: "Delete document" }),
    );
    expect(await screen.findByText("Forbidden")).toBeInTheDocument();
  });
});